...
def find_optimum_tdoa_ch2(file = "recording-beacon-50cm3.wav", Lhat_bounds = (2000, 4000, 50), start_threshold_bounds = (0.001, 0.45, 100), recordings_path = "./Finished recordings/", original_file_path='audio-beacon.wav', Fs_RX = 48000, peak_detection_method="abs", start_detection_method="real", log=False): 
    ...
    for start_threshold in np.linspace(*start_threshold_bounds):
        ...
        for Lhat in np.linspace(*Lhat_bounds).astype(int):
            result = tdoa(x, y, Lhat, method="ch2", start_threshold=start_threshold, Fs_RX=Fs_RX, file=file, peak_detection_method=peak_detection_method, start_detection_method=start_detection_method)
            result.params = {'Lhat': Lhat, 'start_threshold': start_threshold, 'peak_detection_method': peak_detection_method, 'start_detection_method': start_detection_method}
